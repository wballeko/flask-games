...
from domain.tictactoe import TicTacToeGame
from infrastructure.repositories.tictactoe_repository import TicTacToeRepository

BLUEPRINT_NAME = "tictactoe"

tictactoe_routes = Blueprint(BLUEPRINT_NAME, __name__, url_prefix=f"/{BLUEPRINT_NAME}")
...
def index():
    games = repo.get_all()
    return render_template(f"{BLUEPRINT_NAME}/index.html", games=games)
...
def create():
    vs_com = "vs_com" in request.form

    game = service.create_game(
        request.form["player_1_name"], request.form["player_2_name"], vs_com
    )
